...
{#if isLink}
	<a href={isLink ? path : null} on:mouseenter={() => (isHovered = true)} on:mouseleave={() => (isHovered = false)} class="button" style:color style:background-color={backgroundColor}>
		{text}
	</a>
{:else}
	<button on:mouseenter={() => (isHovered = true)} on:mouseleave={() => (isHovered = false)} class="button" style:color style:background-color={backgroundColor}>
		{text}
	</button>
{/if}
